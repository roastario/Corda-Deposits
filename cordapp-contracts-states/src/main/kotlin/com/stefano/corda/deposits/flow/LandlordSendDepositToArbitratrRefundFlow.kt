...
object LandlordSendDepositToArbitratrRefundFlow {
    ...
    class Initiator(val depositId: UniqueIdentifier) : FlowLogic<SignedTransaction>() {
        ...
        override fun call(): SignedTransaction {
            ...
            require(landlord == ourIdentity) { "Arbitration of a deposit must be initiated by the tenant." }

            val sendToArbitrator = Command(
                    DepositContract.Commands.SendToArbitrator(refAndState.state.data.propertyId),
                    listOf(landlord, tenant, scheme).map { it.owningKey }
            )

            progressTracker.currentStep = UPDATING_STATE;

            val copy = refAndState.state.data.copy(sentToArbiter = Instant.now())

            val txBuilder = TransactionBuilder(notary)
                    .addInputState(refAndState)
                    .addOutputState(copy, DepositContract.DEPOSIT_CONTRACT_ID)
                    .addCommand(sendToArbitrator)
